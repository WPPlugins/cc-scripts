...
if ( ! class_exists( __NAMESPACE__ . '\Scripts' ) ) {
	...
	class Scripts extends Scripts\Plugin {
		...
		public function activation() {
			if ( ! self::get_option() ) {
				self::add_option( array(
					...
					'body'   => array( 'scripts' => '', 'method' => 'wp_body_open', 'dependencies' => '', 'minify' => true ),
		...
		public function action_admin_init() {
			...
			add_settings_field( $field = 'body\method\wp_body_open', '', array(
					$this,
					'input'
				), 'scripts', 'scripts', array(
					'field'   => $field,
					...
					'value'   => 'wp_body_open',
					'checked' => 'wp_body_open' == self::get_body( 'method', 'wp_body_open' ) ? 'checked' : '',
					'before'  => '',
					'after'   => 'wp_body_open',
					'desc'    => sprintf(
						self::__( 'Add the following code directly after the %s tag in your theme (preferred method)' ) . ': ' .
						'<code>' . htmlspecialchars( "<?php do_action( 'wp_body_open' ); ?>" ) . '</code>',
		...
		public function sanitize( $option = array() ) {
			return array(
				...
				'body' => array(
					...
					'method'       => in_array( $option['body']['method'], array( 'wp_body_open', 'output_buffering' ) ) ? $option['body']['method'] : null
				),
				'footer' => array(
		...
		public function action_wp_body_open() {
			...
			if ( 'wp_body_open' == self::get_body( 'method', 'wp_body_open' ) && $scripts = self::get_scripts( 'body' ) ) {
